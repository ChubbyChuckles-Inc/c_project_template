...
extern boolean betray_plugin_callback_event_pump(BInputState *input, HWND hWnd, UINT uMsg,
...
extern uint betray_plugin_pointer_allocate(uint user_id, uint device_id, uint button_count, float x,
                                           float y, float z, float *origin, char *name,
                                           boolean draw);
extern void betray_plugin_pointer_set(uint id, float x, float y, float z, float *origin,
                                      boolean *buttons);
...
void betray_timer_debug(char *text)
{
    ...
    printf(
        "timer = %f %s\n",
        (float) imagine_delta_time_compute(new_seconds, new_fractions, old_seconds, old_fractions),
        text);
...
{
    uint array[] = {256,   /*B_SF_USER_COUNT_MAX*/
...
void betray_requester_save_func(void *data)
{
    ...
    if (GetSaveFileName(&ofn))
    {
        ...
        {
            for (j = i; j > 0 && b_win32_requester_save[j] != '.' &&
                        b_win32_requester_save[j] != '/' && b_win32_requester_save[j] != '\\';
                 j--)
                ;
 ...
 */
#define BETRAY_TOUCHINPUTMASKF_TIMEFROMSYSTEM                                                      \
    0x0001 // the dwTime field contains a system generated value
...
BOOL(WINAPI *betray_GetTouchInputInfo)(LPARAM hTouchInput, uint cInputs, BetrayWinTouch *pInputs,
                                       int cbSize) = NULL;
...
LONG WINAPI WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
               ...
               sizeof(RAWINPUTHEADER)) == dwSize);
                    {
                        ...
                        {
                            uint i;
                            for(i = 0; i < raw_pointer_count && raw_pointers[i].hardware_device !=
               raw->header.hDevice; i++); if(i == raw_pointer_count)
                            {
                                if(i == 16)
                                    return;
                                ...
                                if(i != 0)
                                {
                                    raw_pointers[i].device_id =
               betray_plugin_input_device_allocate(betray_user_id, "Pointer");
                                    raw_pointers[i].pointer_id =
               betray_plugin_pointer_allocate(betray_user_id, betray_device_mouse_id, 16, 0.0, 0.0,
               "mouse"); }else
                                {
                            ...
                            input->pointers[i].pointer_x += (float)raw->data.mouse.lLastX /
               (float)b_window_size_x; input->pointers[i].pointer_y -= (float)raw->data.mouse.lLastY
               / (float)b_window_size_x; if(input->pointers[i].pointer_x > 1.0)
                                input->pointers[i].pointer_x = 1.0;
                            if(input->pointers[i].pointer_x < -1.0)
                                input->pointers[i].pointer_x = -1.0;
                            if(input->pointers[i].pointer_y > (float)b_window_size_y /
               (float)b_window_size_x) input->pointers[i].pointer_y = (float)b_window_size_y /
               (float)b_window_size_x; if(input->pointers[i].pointer_y < -(float)b_window_size_y /
               (float)b_window_size_x) input->pointers[i].pointer_y = -(float)b_window_size_y /
               (float)b_window_size_x; if(raw->data.mouse.usButtonFlags & RI_MOUSE_LEFT_BUTTON_DOWN)
        ...
        case WM_NCMBUTTONUP:
            //		case WM_XBUTTONUP :
            //		case WM_XBUTTONDOWN :
            /*		input->pointers[0].pointer_x = (float)(0x0000FFFF | lParam) /
               (float)b_window_size_x; input->pointers[0].pointer_y = (float)((0xFFFF0000 | lParam)
               / 0x0000FFFF) / (float)b_window_size_y;
            */

            if (LOWORD(lParam) < b_window_size_x)
                input->pointers[0].pointer_x =
                    (float) ((signed short) LOWORD(lParam)) / (float) b_window_size_x * 2.0 - 1.0;
            if (HIWORD(lParam) < b_window_size_y)
                input->pointers[0].pointer_y = (1.0 - (float) ((signed short) HIWORD(lParam)) /
                                                          (float) b_window_size_y * 2.0) *
                                               (float) b_window_size_y / (float) b_window_size_x;
            if ((GetMessageExtraInfo() & 0xFFFFFF00) != 0xFF515700)
            {
                ...
                if (input->pointers[0].button[0] || input->pointers[0].button[1] ||
        ...
        case BETRAY_WM_TOUCH:
            if (betray_GetTouchInputInfo != NULL && betray_CloseTouchInputHandle != NULL)
            {
                ...
                if (ids == NULL)
                {
                    betray_device_touch_id =
                        betray_plugin_input_device_allocate(betray_user_id, "Touch");
                ...
                if (betray_GetTouchInputInfo(lParam, (int) wParam, ti, sizeof *ti))
                {
                    for (i = 0; i < wParam; i++)
                    {
                        //	if((ti[i].dwFlags & BETRAY_TOUCHEVENTF_PRIMARY))
                        {
                            ...
                            y = (1.0 - ((float) ti[i].y / 100.0) / (float) b_window_size_y * 2.0) *
                                (float) b_window_size_y / (float) b_window_size_x;
                            if (ti[i].dwFlags & BETRAY_TOUCHEVENTF_DOWN)
                            {
                                j      = betray_plugin_pointer_allocate(betray_user_id,
                                                                        betray_device_touch_id, 1, x, y,
                                                                        -1, NULL, "Touch", FALSE);
                                ids[j] = ti[i].dwID;
                                button = TRUE;
                            ...
                            {
                                for (j = 1;
                                     j < betray_support_functionality(B_SF_POINTER_COUNT_MAX) &&
                                     ids[j] != ti[i].dwID;
                                     j++)
                                    ;
...
HWND betray_plugin_windows_window_handle_get() { return hWnd; }

HDC betray_plugin_windows_device_context_handle_get() { return hDC; }
...
{
    HGLRC(APIENTRY * betray_wglCreateContextAttribsARB)(HDC hDC, HGLRC hshareContext,
                                                        const int *attribList);
    void     *context;
    const int attrib_list[] = {
        WGL_CONTEXT_MAJOR_VERSION_ARB, 2, WGL_CONTEXT_MINOR_VERSION_ARB, 0, WGL_CONTEXT_FLAGS_ARB,
        WGL_CONTEXT_DEBUG_BIT_ARB,     0};
    /*	const int attrib_list[] = {WGL_CONTEXT_MAJOR_VERSION_ARB, 4,
                                WGL_CONTEXT_MINOR_VERSION_ARB, 5,
                                WGL_CONTEXT_FLAGS_ARB, WGL_CONTEXT_CORE_PROFILE_BIT_ARB,
                                0};*/
    betray_wglCreateContextAttribsARB =
        (void *(__stdcall *) (struct HDC__ *, HGLRC hshareContext, const int *attribList))
...
boolean b_init_display_opengl(uint size_x, uint size_y, boolean fullscreenflag, uint samples,
                              char *title, boolean *sterioscopic)
{
    ...
    if (samples > 1)
    {
        GLvoid(APIENTRY * b_wglChoosePixelFormatARB)(HDC hdc, const int *piAttribIList,
                                                     const FLOAT *pfAttribFList, uint nMaxFormats,
                                                     int *piFormats, uint *nNumFormats) = NULL;
        uint    i, count = 0;
        int     format[24];
        boolean output;
        output = b_init_display_opengl(size_x, size_y, fullscreenflag, 1, title, sterioscopic);
        b_wglChoosePixelFormatARB =
            (void(__stdcall *)(struct HDC__ *, const int *, const float *, unsigned int, int *,
                               unsigned int *)) wglGetProcAddress("wglChoosePixelFormatARB");
        if (b_wglChoosePixelFormatARB != NULL)
        {
            int attrib_list[] = {WGL_DRAW_TO_WINDOW_ARB,
                                 GL_TRUE,
    ...
    if (b_win32_instance == NULL)
    {
        ...
        {
            MessageBox(NULL, "BETRAY Error: Failed To Register The Window Class.", "Betray ERROR",
    ...
    if (fullscreenflag)
    {
        ...
        uni_class_name[i] = 0;
        hWnd              = CreateWindowEx(
            WS_EX_APPWINDOW, class_name, uni, WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, 0, 0,
            WindowRect.right - WindowRect.left, WindowRect.bottom - WindowRect.top, NULL, NULL,
            b_win32_instance, NULL);
#else
        hWnd = CreateWindowEx(
            WS_EX_APPWINDOW, class_name, title, WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, 0, 0,
            WindowRect.right - WindowRect.left, WindowRect.bottom - WindowRect.top, NULL, NULL,
            b_win32_instance, NULL);
#endif
    }
    else
    {
        ...
        uni_class_name[i] = 0;
        hWnd =
            CreateWindowEx(WS_EX_APPWINDOW /* | WS_EX_WINDOWEDGE */, class_name, uni,
                           WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
                           (b_win32_display_size_x - (WindowRect.right - WindowRect.left)) / 2,
                           (b_win32_display_size_y - (WindowRect.bottom - WindowRect.top)) / 2,
                           WindowRect.right - WindowRect.left, WindowRect.bottom - WindowRect.top,
                           NULL, NULL, b_win32_instance, NULL);
#else
        hWnd =
            CreateWindowEx(WS_EX_APPWINDOW | WS_EX_WINDOWEDGE, class_name, title,
                           WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
                           (b_win32_display_size_x - (WindowRect.right - WindowRect.left)) / 2,
                           (b_win32_display_size_y - (WindowRect.bottom - WindowRect.top)) / 2,
                           WindowRect.right - WindowRect.left, WindowRect.bottom - WindowRect.top,
    ...
    if (!hWnd)
    {
        MessageBox(NULL, "BETRAY Error: Window Creation Error.", "ERROR",
                   MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
    }
    ...
    if (!(hDC = GetDC(hWnd)))
    {
        MessageBox(NULL, "BETRAY Error: Can't Create A GL Device Context.", "ERROR",
                   MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
    }
    ...
    if (PixelFormat == 0)
    {
        if (!*sterioscopic || 0 == (PixelFormat = ChoosePixelFormat(hDC, &pfd)))
        {
            ...
            if (PixelFormat == 0)
            {
                printf("BETRAY Error: ChoosePixelFormat() failed: Cannot find a suitable pixel "
                       "format.\n");
                return FALSE;
            }
            *sterioscopic = FALSE;
    ...
    {
        MessageBox(NULL, "BETRAY Error: Can't Set The PixelFormat.", "ERROR",
                   MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
    }
#ifdef BETRAY_CONTEXT_OPENGL
    ...
    {
        MessageBox(NULL, "BETRAY Error: Can't Create A GL Rendering Context.", "ERROR",
                   MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
    }
    ...
    {
        MessageBox(NULL, "BETRAY Error: Can't Activate The GL Rendering Context.", "ERROR",
                   MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
    }
#endif
...
#ifdef UNICODE
    {
        ...
        if (dll != NULL)
        {
            betray_GetTouchInputInfo =
                (int(__stdcall *)(long, unsigned int, BetrayWinTouch *, int)) GetProcAddress(
                    dll, "GetTouchInputInfo");
            betray_CloseTouchInputHandle =
                (int(__stdcall *)(long)) GetProcAddress(dll, "CloseTouchInputHandle");
            betray_RegisterTouchWindow = (int(__stdcall *)(
                struct HWND__ *, unsigned long)) GetProcAddress(dll, "RegisterTouchWindow");
        }
    }
#else
    {
...
boolean b_win32_init_display_opengles2(uint size_x, uint size_y, boolean fullscreenflag,
                                       uint samples, char *title, boolean *sterioscopic)
{
    PIXELFORMATDESCRIPTOR pfd;
    GLuint                PixelFormat;
    WNDCLASS              wc;
    ...
    EGLint aEGLAttributes[] = {EGL_RED_SIZE,
                               8,
                               EGL_GREEN_SIZE,
                               8,
                               EGL_BLUE_SIZE,
    ...
    sEGLSurface =
        eglCreateWindowSurface(sEGLDisplay, aEGLConfigs[0], (EGLNativeWindowType) hWnd, NULL);
    ...
    sEGLContext =
        eglCreateContext(sEGLDisplay, aEGLConfigs[0], EGL_NO_CONTEXT, aEGLContextAttributes);
...
#ifdef UNICODE
    {
        ...
        if (dll != NULL)
        {
            ...
            for (i = 0; i < 32 && text[i] != 0; i++)
                u_text[i] = (char) text[i];
            u_text[i]                = 0;
            betray_GetTouchInputInfo = (int(__stdcall *)(long, unsigned int, BetrayWinTouch *,
                                                         int)) GetProcAddress(dll, u_text);
            ...
            for (i = 0; i < 32 && text[i] != 0; i++)
                u_text[i] = (char) text[i];
            u_text[i] = 0;
            betray_RegisterTouchWindow =
                (int(__stdcall *)(struct HWND__ *, unsigned long)) GetProcAddress(dll, u_text);
        }
        exit(0);
    }
#else
...
void betray_device_init()
{
    ...
    betray_mouse_id = betray_plugin_pointer_allocate(betray_user_id, betray_device_mouse_id,
                                                     GetSystemMetrics(SM_CMOUSEBUTTONS), 0.0, 0.0,
                                                     -1, NULL, "mouse", FALSE);
}

void system_wrapper_lose_focus(void) { input_focus = FALSE; }

void betray_set_mouse_warp(boolean warp) { mouse_warp = warp; }
...
void APIENTRY betray_glBindFramebufferEXT(GLenum target, GLuint framebuffer)
{
    ...
    if (internal_glBindFramebufferEXT == NULL)
        internal_glBindFramebufferEXT =
            (void(APIENTRY __stdcall *)(GLenum, GLuint)) wglGetProcAddress("glBindFramebufferEXT");
    if (framebuffer == 0)
...
void betray_end_of_frame_flush() /* leaking memeory! */
{
    ...
    if (fence == ~0)
    {
        b_glFenceSync = (GLsync(__stdcall *)(GLenum condition, GLbitfield flags)) wglGetProcAddress(
            "glFenceSync");
        b_glClientWaitSync = (GLenum(__stdcall *)(
            GLsync sync, GLbitfield flags, uint64 timeout)) wglGetProcAddress("glClientWaitSync");
...
void betray_launch_main_loop(void)
{
    ...
    /*	{
            ...
            Rid[0].usUsage = 0x02;
            Rid[0].dwFlags = RIDEV_NOLEGACY;   // adds HID mouse and also ignores legacy mouse
       messages Rid[0].hwndTarget = 0; RegisterRawInputDevices(Rid, 1, sizeof(Rid[0]));
        }*/
    ...
    while (!window_close)
    {
        ...
        else // If There Are No Messages
        {
            ...
            if (mouse_warp_move)
            {
                ...
                SetCursorPos(
                    current_pos.x + (int) (mouse_warp_move_x * (float) b_window_size_x * 0.5),
                    current_pos.y + (int) (mouse_warp_move_y * (float) b_window_size_y * 0.5));
                mouse_warp_move = FALSE;
            }
            /*	if(mouse_warp)
                    SetCursorPos(b_window_center_x, b_window_center_y);
                GetCursorPos(&current_pos);
                input->pointers[0].pointer_x = (float)(current_pos.x - b_window_center_x) /
               (float)b_window_size_x * 2.0; betray_screen_mode_get(NULL, NULL, &fullscreen);
                if(fullscreen)
                    input->pointers[0].pointer_y = (float)(current_pos.y - b_window_center_y) /
               (float)b_window_size_x * -2.0; else input->pointers[0].pointer_y =
               (float)(current_pos.y - b_window_center_y - 12) / (float)b_window_size_x * -2.0;*/
